...
test.describe('API — 内容类型与错误处理', () => {
  ...
  test('GET /api/v4/user/me 未认证返回 401', async ({ request }) => {
    const res = await request.get(`${BASE}/api/v4/user/me`);
    expect(res.status()).toBe(401);
  });

  test('POST /api/v4/user/register 缺少参数返回业务错误', async ({ request }) => {
    const res = await request.post(`${BASE}/api/v4/user/register`, { data: {} });
    expect(res.status()).toBeGreaterThanOrEqual(200);
    const body = await res.json();
    expect(typeof body.code).toBe('number');
  });

  test('GET /api/v4/image/works 未认证限流', async ({ request }) => {
  ...
  test('POST JSON content-type 正确解析', async ({ request }) => {
    const res = await request.post(`${BASE}/api/v4/user/login`, {

    ...
    <div class="toast-container" role="alert" aria-live="polite">
      <TransitionGroup name="toast">
        <div v-for="t in toasts" :key="t.id" class="toast-item" :class="t.type">
          ...
          <button v-if="t.closable" class="toast-close" @click="remove(t.id)" :aria-label="$t('common.close')">✕</button>
        </div>
      </TransitionGroup>
    </div>
  </Teleport>
...
<style scoped>
.toast-container { position: fixed; top: 70px; right: 20px; z-index: var(--z-toast, 6000); display: flex; flex-direction: column; gap: 8px; pointer-events: none; }
.toast-item {

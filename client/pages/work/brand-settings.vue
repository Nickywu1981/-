  ...
  <WorkLayout title="品牌统一配置" subtitle="一键设置品牌VI/水印/角标，全局统一应用到全部素材" :steps="steps" :current-step="currentStep">
    <template #input>
      <div class="ws-section"><div class="ws-section__title">品牌信息</div>
        ...
        <div class="form-group"><label>品牌Logo</label><div class="upload-zone" @click="uploadLogo"><span v-if="!form.logoUrl">📷 上传Logo</span><img v-else :src="form.logoUrl" style="max-width:120px;max-height:60px" /></div></div></div>
      </div>
...
const saveSettings = async () => { task.status=1; task.progressMsg='保存中...'; try { await $fetch('/api/brand',{method:'PUT',body:{name:form.brandName,logo:form.logoUrl}}); task.status=2 } catch(e) { errorMsg.value=e.message } }
const uploadLogo = () => {}
</script>

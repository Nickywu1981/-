...
function handleDrop(e: DragEvent) { e.preventDefault(); if (e.dataTransfer?.files[0]) { previewUrl.value = URL.createObjectURL(e.dataTransfer.files[0]); uploadFile(e.dataTransfer.files[0]) } }
...
.mode-card { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 20px 10px; border: 2px solid var(--input-border); border-radius: var(--radius-lg); background: var(--bg-card); cursor: pointer; transition: all var(--transition-fast); }

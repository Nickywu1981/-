...
function handleDrop(e: DragEvent) { e.preventDefault(); if (e.dataTransfer?.files[0]) { previewUrl.value = createBlobUrl(e.dataTransfer.files[0]); uploadFile(e.dataTransfer.files[0]) } }
...
.mode-card { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 20px 10px; border: 2px solid var(--input-border); border-radius: var(--radius-lg); background: var(--bg-card); cursor: pointer; transition: border-color var(--transition-fast), transform var(--transition-fast), box-shadow var(--transition-fast); }
.mode-card:hover { border-color: var(--brand-light); transform: translateY(-2px); box-shadow: var(--shadow-md); }
.mode-card.active { border-color: var(--brand); background: var(--brand-subtle); box-shadow: var(--shadow-brand); }
...
.result-image { max-width: 100%; max-height: 400px; border-radius: var(--radius-xl); box-shadow: var(--shadow-lg); margin-bottom: 12px; }
.result-hint { font-size: 12px; color: var(--text-muted); margin-bottom: 16px; }

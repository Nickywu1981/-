...
async function fetchAll() {
  ...
  try {
    ...
    if (activeTab.value === 'all' || activeTab.value === 'image') {
      endpoints.push($fetch('/api/images/tasks', { params: { page: page.value, pageSize }, credentials: 'include' }).catch(() => ({ list: [], total: 0 })));
    }
    if (activeTab.value === 'all' || activeTab.value === 'video') {
      endpoints.push($fetch('/api/videos/tasks', { params: { page: page.value, pageSize }, credentials: 'include' }).catch(() => ({ list: [], total: 0 })));
    }
    if (activeTab.value === 'all' || activeTab.value === 'batch') {
      endpoints.push($fetch('/api/advanced/tasks', { params: { page: page.value, pageSize }, credentials: 'include' }).catch(() => ({ list: [], total: 0 })));
      endpoints.push($fetch('/api/adv-video/tasks', { params: { page: page.value, pageSize }, credentials: 'include' }).catch(() => ({ list: [], total: 0 })));
    }

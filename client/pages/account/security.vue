<template>
  <div class="account-page"><h1>安全设置</h1>
    ...
    <div class="security-section"><h3>两步验证</h3><p>增强账户安全性</p><button class="btn-secondary">开启两步验证</button></div>
    <div class="security-section"><h3>登录设备</h3><p v-if="!devices.length" class="empty-hint">暂无设备记录</p><ul v-else><li v-for="d in devices" :key="d.id">{{ d.device }} — {{ d.location }} — {{ d.time }}</li></ul></div>
  </div>
</template>
...
.btn-secondary { padding: 10px 24px; background: var(--bg-card); color: var(--brand); border: 1px solid var(--brand); border-radius: var(--radius-md); cursor: pointer; font-size: 14px; }

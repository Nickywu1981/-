    ...
    <Teleport to="body">
      <div v-if="modalOpen" class="modal-overlay" @click.self="modalOpen = false">
        <div class="modal">
          ...
          <div class="form-grid">
            <label class="full">标题 <input v-model="form.title" placeholder="表单标题" /></label>
            <label>编码 <input v-model="form.form_code" placeholder="唯一编码" /></label>
...
async function delForm(id: number) {
  if (!confirm('确认删除该表单？此操作不可恢复。')) return
  try {

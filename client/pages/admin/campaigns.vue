...
async function fetch() {
  ...
  try {
    ...
    const res = await $api(`/admin/campaign/campaigns?${params}`)
...
async function save() {
  ...
  try {
    if (editing.value) {
      await $api(`/admin/campaign/campaigns/${editing.value.id}`, { method: 'PUT', body: form })
    } else {
      await $api('/admin/campaign/campaigns', { method: 'POST', body: form })
    }

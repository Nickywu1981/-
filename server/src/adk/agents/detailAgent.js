...
const modulesTool = new FunctionTool('create_detail_modules', async (params) => {
  const modules = [
    { key: 'header_banner', label: '头图banner', prompt: `e-commerce product detail page header banner, ${params.productName}, professional layout, brand style, 750x600` },
    { key: 'feature_showcase', label: '核心卖点展示', prompt: `e-commerce product feature showcase module, ${params.productName}, key features: ${params.sellingPoints || 'high quality'}, clean design, 750x800` },
    { key: 'specs_table', label: '规格参数', prompt: `e-commerce product specifications table module, ${params.productName}, specs: ${params.specs || 'standard'}, infographic style, 750x600` },
    { key: 'detail_showcase', label: '细节展示', prompt: `e-commerce product detail showcase, ${params.productName}, material and craftsmanship close-up, 750x800` },
    { key: 'scene_demo', label: '场景演示', prompt: `e-commerce product lifestyle scene demonstration, ${params.productName} in real usage scenario, 750x800` },
    { key: 'footer_cta', label: '底部行动号召', prompt: `e-commerce product detail page footer with CTA, ${params.productName}, buy now button, trust badges, 750x400` },
  ];

  return batchImageGen(modules, { source: 'detail_module' });
}, {

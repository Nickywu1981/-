...
export const submitVirtualTryon = wrapController(async (req, res) => {
    ...
    if (!productImageUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传服装图');
...
export const submitStyleTransfer = wrapController(async (req, res) => {
    ...
    if (!productImageUrl || !targetStyle) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传产品图并选择目标风格');
...
export const submitImageTranslate = wrapController(async (req, res) => {
    ...
    if (!productImageUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传包含文字的图片');
    if (!targetLang) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请选择目标语言');
...
export const submitOutpainting = wrapController(async (req, res) => {
    ...
    if (!productImageUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传需要扩图的图片');
...
export const submitGhostMannequin = wrapController(async (req, res) => {
    ...
    if (!productImageUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传假模服装图');
...
export const submitModelGenerate = wrapController(async (req, res) => {
    ...
    if (!imageUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传商品图片');
...
export const submitSwapFace = wrapController(async (req, res) => {
    ...
    if (!baseUrl || !faceUrl) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请上传底图和面部图片');
...
export const submitTextEffect = wrapController(async (req, res) => {
    ...
    if (!text) throw new BusinessError(ERROR_CODE.PARAM_MISSING, '请输入文字内容');

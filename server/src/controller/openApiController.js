...
export const ping = wrapController(async (req, res) => { success(res, await openApiService.ping(req.tenantId)); });

export const getUsage = wrapController(async (req, res) => { success(res, await openApiService.getUsage(req.tenantId, req.apiKeyRecord)); });

export const removeBackground = wrapController(async (req, res) => {
    const result = await openApiService.removeBackground(req.tenantId, req.body.imageUrl);
    success(res, result);
});

export const generateScene = wrapController(async (req, res) => {
    const result = await openApiService.generateScene(req.tenantId, req.body.imageUrl, req.body.sceneType);
    success(res, result);
});

export const retouchImage = wrapController(async (req, res) => {

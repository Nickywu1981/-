...
export async function getGroupConfig(groupKey, userId, userRole) {
  ...
  if (userLevel < getPermissionLevel(groupKey)) throw new BusinessError(ERROR_CODE.FORBIDDEN);
...
export async function setConfig(groupKey, itemKey, itemValue, changedBy) {
  ...
  if (oldValue === null) throw new BusinessError(ERROR_CODE.RESOURCE_NOT_FOUND);
...
export async function rollbackConfig(logId, changedBy) {
  ...
  if (!log) throw new BusinessError(ERROR_CODE.RESOURCE_NOT_FOUND);

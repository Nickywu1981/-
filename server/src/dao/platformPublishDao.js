...
export async function saveCredential(userId, { platform, appKey, appSecret, accessToken, shopName }) {
...
export async function getCredential(userId, platform) {
  const [rows] = await db.query(
    'SELECT id, user_id, platform, app_key, app_secret, access_token, shop_name, status, token_expires_at, created_at, updated_at FROM platform_credentials WHERE user_id = ? AND platform = ? AND status = ? LIMIT 1',
...
export async function getPlatformConfigs() {
  const [rows] = await db.query(
    'SELECT id, platform, config_name, config_value, description, status, created_at, updated_at FROM platform_config WHERE status = ? ORDER BY id LIMIT 200', ['active'],
  );
  return rows;
}

export async function createPublishRecord(userId, { platform, title, description, images, price, itemId }) {
...
export async function updatePublishStatus(id, userId, status, resultUrl = null, errorMsg = null) {
  const [r] = await db.query(
...
export async function getPublishHistory(userId, { platform, page = 1, limit = 20 } = {}) {
  const { offset } = parsePagination({ page, pageSize: limit });
  let sql = 'SELECT id, user_id, platform, title, description, images, price, item_id, status, result_url, error_msg, created_at, updated_at FROM platform_publish_history WHERE user_id = ?';
  const params = [userId];

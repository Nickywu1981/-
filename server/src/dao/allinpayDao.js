...
export async function getByReqsn(reqsn) {
  const [rows] = await pool.query(`SELECT ${COLS} FROM allinpay_order WHERE reqsn = ? LIMIT 1`, [reqsn]);
  return rows[0] || null;
}

export async function markPaid(reqsn, trxid, notifyRaw, conn) {
  const db = conn || pool;
...
export async function markFailed(reqsn) {
  const [r] = await pool.query(
...
export async function logNotify(data) {
  await pool.query(
...
export async function isCallbackProcessed(reqsn, trxid) {

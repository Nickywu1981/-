...
export async function getFormById(id, tenantId) {
  const [rows] = await pool.query('SELECT id, tenant_id, title, form_code, description, fields_json, pc_config, mobile_config, submit_limit, submit_count, max_submissions_per_user, start_time, end_time, status, success_msg, redirect_url, data_retention_days, notify_email, access_type, owner_id, create_time, update_time FROM custom_form WHERE id = ? AND tenant_id = ? LIMIT 1', [id, tenantId]);
  return rows[0] || null;
}

export async function getFormByCode(code, tenantId) {
  const [rows] = await pool.query('SELECT id, tenant_id, title, form_code, description, fields_json, pc_config, mobile_config, submit_limit, submit_count, max_submissions_per_user, start_time, end_time, status, success_msg, redirect_url, data_retention_days, notify_email, access_type, owner_id, create_time, update_time FROM custom_form WHERE form_code = ? AND tenant_id = ? AND status = 1 LIMIT 1', [code, tenantId]);
  return rows[0] || null;
}
...
export async function updateForm(id, tenantId, fields) {
  const allowed = [
...
export async function incrementSubmitCount(formId) {
  await pool.query('UPDATE custom_form SET submit_count = submit_count + 1 WHERE id = ?', [formId]);
}

export async function incrementSubmitCountAtomic(formId, submitLimit) {
  const [r] = await pool.query(
...
export async function getUserSubmissionCount(formId, userId) {
  const [rows] = await pool.query(
...
export async function addSubmission({ formId, tenantId, userId, dataJson, ip, userAgent, deviceType, ipLong }) {
  const [r] = await pool.query(
...
export async function listSubmissions(formId, { page = 1, pageSize = 50, status, dataStatus, startDate, endDate }) {
  let sql = 'SELECT id, form_id, tenant_id, user_id, data_json, status, data_status, remark, ip, user_agent, device_type, create_time, update_time FROM custom_form_submission WHERE form_id = ?';
  const params = [formId];
  const { offset } = parsePagination({ page, pageSize });
...
export async function getAllSubmissions(formId, { page = 1, pageSize = 1000 } = {}) {
  const { offset } = parsePagination({ page, pageSize });
  const [rows] = await pool.query('SELECT id, form_id, tenant_id, user_id, data_json, status, data_status, remark, ip, user_agent, device_type, create_time, update_time FROM custom_form_submission WHERE form_id = ? ORDER BY create_time DESC LIMIT ? OFFSET ?', [formId, pageSize, offset]);
  return rows;
}

export async function updateSubmission(id, formId, fields) {
...
export async function listFields(formId, tenantId) {
  const [rows] = await pool.query(
    'SELECT id, form_id, tenant_id, field_name, field_label, field_type, sort_order, is_required, is_visible, default_value, placeholder, options_json, validation_rules, linkage_conditions, linkage_action, masking_rule, masking_pattern, pc_col_span, mobile_col_span, css_class, create_time, update_time FROM diy_custom_field WHERE form_id = ? AND tenant_id = ? ORDER BY sort_order LIMIT 200',
    [formId, tenantId],
  );
  return rows;
}

export async function deleteFields(formId, tenantId, conn) {
  const db = conn || pool;
  await db.query('DELETE FROM diy_custom_field WHERE form_id = ? AND tenant_id = ?', [formId, tenantId]);
}

export async function batchInsertFields(tenantId, formId, fields, conn) {

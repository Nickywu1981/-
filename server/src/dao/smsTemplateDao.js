...
export async function findByCode(templateCode) {
  const [rows] = await pool.execute(
    'SELECT id, template_code, name, content, params_count, provider_template_id, provider, status, remark, create_time FROM sms_template WHERE template_code = ? AND status = 1 LIMIT 1',
    [templateCode],
  );
  return rows[0] || null;
}
...
export async function updateTemplate(id, fields) {
  ...
  const [rows] = await pool.execute('SELECT id, template_code, name, content, params_count, provider_template_id, provider, status, remark, create_time FROM sms_template WHERE id = ? LIMIT 1', [id]);
  return rows[0] || null;
}

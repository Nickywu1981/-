...
export async function listCampaigns({ page = 1, pageSize = 20, type, status } = {}) {
  const conditions = [];
  const params = [];
...
export async function updateCampaign(id, tenantId, data) {
...
export async function deleteCampaign(id, tenantId) {
...
export async function listCoupons({ page = 1, pageSize = 20, status, campaignId } = {}) {
  const conditions = [];
  const params = [];
  if (status !== undefined) { conditions.push('status = ?'); params.push(status); }
...
export async function updateCoupon(id, campaignId, data) {
...
export async function deleteCoupon(id, campaignId) {
  const [result] = await pool.query('DELETE FROM coupon WHERE id = ? AND campaign_id = ?', [id, campaignId]);
  return result.affectedRows > 0;
}

export async function getUserCoupons({ page = 1, pageSize = 20, userId, status } = {}) {
  const conditions = [];
  const params = [];
...
export async function listAnnouncements({ page = 1, pageSize = 20, type, status } = {}) {
  const conditions = [];
  const params = [];
  if (type) { conditions.push('type = ?'); params.push(type); }
  if (status !== undefined) { conditions.push('status = ?'); params.push(status); }
...
export async function updateAnnouncement(id, createBy, data) {
...
export async function deleteAnnouncement(id, createBy) {

...
export async function updateDecay(namespace, subjectId) {
  const pool = db;
  await db.execute(
    `UPDATE ltm_entries SET recency_score = GREATEST(0.01, recency_score * EXP(-decay_rate * GREATEST(DATEDIFF(NOW(), COALESCE(last_accessed_at, created_at)), 0)))
     WHERE namespace = ? AND subject_id = ? AND is_pinned = 0 AND COALESCE(last_accessed_at, created_at) < DATE_SUB(NOW(), INTERVAL 1 DAY)`,
    [namespace, subjectId]
  );
}
...
export async function getCandidatesForConsolidation({ namespace, subjectId, memoryType, limit = 20 }) {
  ...
  let sql = `SELECT id, content, importance, memory_type FROM ltm_entries
    WHERE namespace = ? AND subject_id = ? AND is_pinned = 0 AND importance < 0.6 AND recency_score < 0.5

...
export async function checkWhitelist(tenantId, url) {
  let hostname;
...
export async function updateWhitelist(id, tenantId, fields) {
...
export async function checkRateLimit(proxyId) {
  const [rows] = await pool.query(
...
export async function setCircuitBreak(proxyId, failCount, tenantId) {
  const [r] = await pool.query(
...
export async function resetCircuit(proxyId, tenantId) {
  const [r] = await pool.query(
...
export async function listLogs({ tenantId, proxyId, startTime, endTime, status, page = 1, pageSize = 20 }) {

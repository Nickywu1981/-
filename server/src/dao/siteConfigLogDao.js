...
export const getLogsByKey = async (configKey, limit = 50) => {
  const [rows] = await pool.query(
    `SELECT id, group_key, item_key, old_value, new_value, changed_by, created_at FROM ${table} WHERE group_key = 'site_config' AND item_key = ? ORDER BY created_at DESC LIMIT ?`,

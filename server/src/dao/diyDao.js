...
export async function listPages(tenantId, { pageType, status, keyword, ownerId, accessType, page = 1, pageSize = 20 }) {
...
export async function getPageById(id, tenantId) {
...
export async function getPagesByIds(ids, tenantId) {
...
export async function getPageBySlug(slug, tenantId) {
  const [rows] = await pool.query(
...
export async function getPublishedPage(slug, tenantId) {
  const redis = getRedis();
  const cacheKey = _cacheKey(slug, tenantId);
  if (redis) {
    try {
...
export async function createPage({ tenantId, ownerId, title, slug, pageType, accessType, mobileConfig, pcConfig, metaJson }) {
...
export async function updatePage(id, tenantId, fields) {
  const oldSlug = fields.slug ? null : await _getSlugById(id, tenantId);
  ...
  if (r.affectedRows) {
    if (oldSlug) await clearPageCache(oldSlug, tenantId);
    if (fields.slug) await clearPageCache(fields.slug, tenantId);
  }
  return r.affectedRows;
}

export async function updateAccessCount(slug, tenantId) {
  if (tenantId) {
    await pool.query('UPDATE diy_page SET access_count = access_count + 1 WHERE slug = ? AND tenant_id = ?', [slug, tenantId]);
  } else {
...
export async function softDeletePage(id, tenantId) {
  const [r] = await pool.query('UPDATE diy_page SET status = 3 WHERE id = ? AND tenant_id = ?', [id, tenantId]);
  if (r.affectedRows) await _clearCacheById(id, tenantId);
  return r.affectedRows;
}

export async function restorePage(id, tenantId) {
  const [r] = await pool.query('UPDATE diy_page SET status = 0 WHERE id = ? AND tenant_id = ? AND status = 3', [id, tenantId]);
  return r.affectedRows;
}

export async function hardDeletePage(id, tenantId) {
  const slug = await _getSlugById(id, tenantId);
  return withTransaction(async (conn) => {
    ...
    if (slug) await clearPageCache(slug, tenantId);
  });
}

export async function unpublishPage(id, tenantId) {
  const [r] = await pool.query('UPDATE diy_page SET status = 2, offline_time = NOW() WHERE id = ? AND tenant_id = ? AND status = 1', [id, tenantId]);
  if (r.affectedRows) await _clearCacheById(id, tenantId);
  return r.affectedRows;
}

export async function republishPage(id, tenantId) {
...
export async function saveVersion(pageId, mobileConfig, pcConfig, { remark, autoSave = false, rollbackFrom = null }) {
  return withTransaction(async (conn) => {
...
export async function listVersions(pageId, { includeAuto = false } = {}) {
...
export async function getVersion(pageId, version) {
  const [rows] = await pool.query(
...
export async function getLatestAutoVersion(pageId) {
  const [rows] = await pool.query(
...
export async function listComponents(tenantId, category, { page = 1, pageSize = 500 } = {}) {
...
function _cacheKey(slug, tenantId) {
  return tenantId ? `${REDIS_KEY_PREFIX}${tenantId}:${slug}` : `${REDIS_KEY_PREFIX}${slug}`;
}

// ==================== Redis 缓存管理 ====================

export async function cachePublishedPage(slug, data, tenantId) {
  const redis = getRedis();
  if (redis) {
    const key = _cacheKey(slug, tenantId);
    try { await redis.setex(key, REDIS_TTL, JSON.stringify(data)); } catch (e) { logger.warn('[DiyDao] cachePublishedPage 缓存写入失败', { slug, tenantId, error: e.message }); }
  }
}

export async function clearPageCache(slug, tenantId) {
  const redis = getRedis();
  if (redis) {
    const key = _cacheKey(slug, tenantId);
    try { await redis.del(key); } catch (e) { logger.warn('[DiyDao] clearPageCache 缓存删除失败', { slug, tenantId, error: e.message }); }
  }
}

async function _getSlugById(id, tenantId) {
  const [rows] = await pool.query('SELECT slug FROM diy_page WHERE id = ? AND tenant_id = ? LIMIT 1', [id, tenantId]);
  return rows[0]?.slug || null;
}

async function _clearCacheById(id, tenantId) {
  const slug = await _getSlugById(id, tenantId);
  if (slug) await clearPageCache(slug, tenantId);
}
...
export async function batchUpdateStatus(ids, tenantId, status) {
  const placeholders = ids.map(() => '?').join(',');
  await pool.query(`UPDATE diy_page SET status = ? WHERE id IN (${placeholders}) AND tenant_id = ?`, [status, ...ids, tenantId]);
}

export async function batchPublishWithVersions(ids, tenantId) {
  if (!ids || !ids.length) return [];
  const placeholders = ids.map(() => '?').join(',');
  return withTransaction(async (conn) => {
...
export async function createPageWithVersion({ tenantId, ownerId, title, slug, pageType, accessType, mobileConfig, pcConfig, metaJson }) {
  return withTransaction(async (conn) => {
...
export async function publishWithVersion(id, tenantId, mobileConfig, pcConfig, slug) {
  return withTransaction(async (conn) => {
...
export async function cloneWithVersion(src, tenantId) {
...
export async function rollbackWithVersion(pageId, tenantId, srcVersion) {
  return withTransaction(async (conn) => {
    const [[{ v }]] = await conn.query('SELECT COALESCE(MAX(version),0)+1 as v FROM diy_page_version WHERE page_id = ? FOR UPDATE', [pageId]);
    await conn.query(
...
export async function listTemplates({ industry, pageType, keyword, page = 1, pageSize = 20 }) {
  const { offset } = parsePagination({ page, pageSize });
...
export async function incrementTemplateUse(id) {
  await pool.query('UPDATE diy_template SET use_count = use_count + 1 WHERE id = ?', [id]);
}

export async function listTemplateIndustries() {
